...
typescrpt继承了JavaScript的类型设计，上面所有类型为小写。

`Number`、`String`、`Boolean`在JavaScript是内置对象，而不是类型名称
...
当在 **类型上下文**（Type Context）中使用 `typeof` 时，它的参数必须是 **标识符**（如变量名或属性名），而不能是表达式或需要运行时计算的复杂操作。这是 TypeScript 类型系统的静态分析特性决定的，因为类型检查发生在编译时，而非运行时。
